...
Les variables actives ont toutes une modalité "non-réponse" ("NA"), qui concerne quelques individus. L'ACM dite "spécifique" permet de neutraliser ces modalités dans la construction de l'espace factoriel, tout en conservant l'ensemble des individus.

```{r nb_na}
sapply(Taste[,1:11], function(x) sum(x=="NA"))
```

On commence par repérer le rang des modalités que l'on souhaite neutraliser.
...
On voit ici que les deux premiers axes capturent l'essentiel de l'information. On va donc dans la suite se concentrer sur le plan formé par les axes 1 et 2.
...
p <- ggadd_supvar(p, mca, Taste$Age, col="dodgerblue3", shape=NULL, segment=FALSE)
p <- ggadd_supvar(p, mca, Taste$Educ, col="tomato", shape=NULL, segment=FALSE)
ggadd_supvar(p, mca, Taste$Gender, col="seagreen", shape=NULL, segment=FALSE)
```
...
Au niveau des modalités, on peut caractériser l'association d'une modalité de variable supplémentaire avec une axe à partir des *coefficients de corrélation*.

```{r condesc1, eval=FALSE}
condesc(mca$ind$coord[,1], Taste[,c("Gender","Age","Educ")])$categories
```

```{r condesc1bis, echo=FALSE}
knitr::kable(condesc(mca$ind$coord[,1], Taste[,c("Gender","Age","Educ")])$categories, row.names=FALSE)
```
...
Les ellipses de concentration ne donnent en revanche qu'une représentation imparfaite de la répartition des points du sous-nuage, du fait de l'éventuelle superposition des points (de manière analogue à ce qu'on a vu pour le nuage des indidividus) et son centrage sur le point moyen. Il peut dès lors être intéressant de compléter une ellipse de concentration par une représentation de la densité des points du sous-nuage, sous forme de contours ou de surfaces.
...
```{r educ_area}
ggadd_density(p, mca, var=Taste$Educ, cat="High", density="area")
```
...
Le problème de la typicalité consiste à se demander si un groupe d'individus peut être assimilé à la population de référence ou s'il est atypique. Un *test de typicalité* calcule une *p-value combinatoire*, qui définit le "degré de typicalité" du point moyen du groupe d'individus. Une p-value faible est considérée comme statistiquement significative au sens combinatoire et traduit une différence qui n'est probablement pas dûe au hasard.
