dimcontrib <- function(resmca,dim=c(1,2),best=TRUE) {
    ...
    #inddesc <- lapply(as.list(dim),function(x) data.frame(ctr=resmca$ind$contrib[,x]*resmca$ind$coord[,x]/abs(resmca$ind$coord[,x])))#,weight=resmca$call$row.w[resmca$call$subcloud])) #ajout de subcloud
