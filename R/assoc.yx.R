assoc.yx <- function(y,x,xx=TRUE,twocont="kendall",nperm=1000,distrib="asympt",dec.a=3,dec.p=3) {
  ...
  if(xx==TRUE) {
    ...
    for(i in 1:length(combi)) {
      ...
      if(class(x1) %in% c('numeric','integer') & class(x2) %in% c('numeric','integer')) {
        z <- GDAtools::assoc.twocont(x1, x2, nperm=nperm, distrib=distrib)
        measure = twocont
        association = z[,twocont][1]
        permutation.pvalue = z[,twocont][2]
      }
      if(class(x1) %in% c('numeric','integer') & class(x2)=='factor') {
        z <- GDAtools::assoc.catcont(x2, x1, nperm=nperm, distrib=distrib)
        measure='Eta2'
        association = z$eta.squared
        permutation.pvalue = z$permutation.pvalue
      }
      if(class(x1)=='factor' & class(x2) %in% c('numeric','integer')) {
        z <- GDAtools::assoc.catcont(x1, x2, nperm=nperm, distrib=distrib)
        measure='Eta2'
        association = z$eta.squared
        permutation.pvalue = z$permutation.pvalue
      }
      if(class(x1)=='factor' & class(x2)=='factor') {
        z <- GDAtools::assoc.twocat(x1, x2, nperm=nperm, distrib=distrib)
        measure="Cramer's V"
        association = z$cramer.v
        permutation.pvalue = z$permutation.pvalue
      }
