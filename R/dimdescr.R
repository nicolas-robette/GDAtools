dimdescr <- function(resmca,vars=NULL,dim=c(1,2),min.cor=NULL,nperm=100,distrib="asympt") {
   ...
   for(i in 1:length(dim)) {
      ...
      if(classe == 'csMCA') temp <- condesc(resmca$ind$coord[,i],X[resmca$call$subcloud,],weights=resmca$call$row.w[resmca$call$subcloud],min.cor=min.cor,nperm=nperm,distrib=distrib,dec=c(3,3,3,3))
      rownames(temp) <- NULL
      res[[i]] <- temp
      }
